 ...
 */
void d39_bt_init(char **err) {
    int ret = bt_le_adv_start(BT_LE_ADV_CONN, ad, ARRAY_SIZE(ad), NULL, 0);
    if (ret) {
        *err = "Could not start advertising";
        return;
    }
...
void d39_bt_send_msg(char *buf, size_t len, char **err) {
    if (!notify_enable) {
        return;
    }
